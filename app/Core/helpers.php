...
function absolute_url(string $path=''):string{
 $settings=config();
 $origin=rtrim((string)($settings['site_url']??''),'/');
 if(!preg_match('#^https?://[^/]+#i',$origin)){
  $host=preg_replace('/[^a-z0-9.:-]/i','',(string)($_SERVER['HTTP_HOST']??''))?:'rutadocente.com';
  $origin='https://'.$host.rtrim((string)($settings['base_url']??''),'/');
 }
