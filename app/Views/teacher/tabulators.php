...
<main class="content container-fluid teacher-library tabulators-library">
 <section class="library-hero tabulator-hero">
  <div class="library-hero-copy"><span class="eyebrow">ANÁLISIS DE RESULTADOS</span><h2>Tabuladores claros para decidir mejor.</h2><p>Organiza resultados, identifica avances y transforma los datos de tus evaluaciones en acciones pedagógicas concretas.</p><div class="library-hero-meta"><span><b><?=count($items)?></b> herramientas</span><span><b><?=count($groups)?></b> grupos</span><span><b><?=e($user['subject']??'General')?></b> asignatura</span></div></div>
 ...
 <?php if(!$enabled):?>
  <section class="library-locked"><span class="locked-icon">▦</span><div><small>MÓDULO NO HABILITADO</small><h2>Tu acceso a tabuladores aún no está activo.</h2><p>Solicita la habilitación para consultar las herramientas de análisis asignadas a tu perfil.</p><a class="btn btn-brand" href="<?=url('/contacto')?>">Solicitar acceso →</a></div></section>
 <?php else:?>
  <section class="library-toolbar">
   <div><span class="module-kicker">HERRAMIENTAS DE ANÁLISIS</span><h2>Mis tabuladores</h2><p>Organizados por grupo para que encuentres rápidamente lo que necesitas.</p></div>
   <label class="library-search"><span>⌕</span><input type="search" data-resource-search placeholder="Buscar un tabulador..." aria-label="Buscar un tabulador"></label>
  </section>
  ...
  <?php if(!$items):?><section class="library-empty"><span>▦</span><h2>Aún no hay tabuladores publicados.</h2><p>Las nuevas herramientas aparecerán aquí cuando sean asignadas a tu área.</p><a href="<?=url('/docente')?>">Volver al inicio →</a></section><?php else:?>
