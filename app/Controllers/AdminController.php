...
class AdminController extends Controller {
 ...
 function catalogForm():void{$this->admin();$type=$_GET['type']??'subjects';$catalog=new Catalog($type);$id=(int)($_GET['id']??0);$labels=['roles'=>'Rol','subjects'=>'Asignatura','tabulator_groups'=>'Grupo de talleres asincrónicos','tabulator_subgroups'=>'Subgrupo de talleres asincrónicos'];$this->view('admin/catalog-form',['title'=>($id?'Editar ':'Nuevo ').$labels[$type],'type'=>$type,'label'=>$labels[$type],'record'=>$id?$catalog->find($id):null,'groups'=>(new Catalog('tabulator_groups'))->all()]);}
 ...
 private function resourceList(string $type):void{$this->admin();$this->view('admin/resources',['title'=>$type==='test'?'Tests':'Talleres asincrónicos','type'=>$type,'resources'=>(new Resource)->detailed($type)]);}
 function resourceForm():void{$this->admin();$id=(int)($_GET['id']??0);$resource=$id?(new Resource)->findDetailed($id):null;$type=$resource['type']??($_GET['type']??'test');if(!in_array($type,['test','tabulador'],true))$type='test';$this->view('admin/resource-form',['title'=>$id?'Editar '.($type==='test'?'test':'taller asincrónico'):'Nuevo '.($type==='test'?'test':'taller asincrónico'),'type'=>$type,'resource'=>$resource,'subjects'=>(new Catalog('subjects'))->all(),'groups'=>(new Catalog('tabulator_groups'))->all(),'subgroups'=>(new Catalog('tabulator_subgroups'))->all()]);}
